...
if local_or_cplab == "local":
    path_to_trimmed_outfiles = '/home/elenaespinosa/OneDrive/Uni/Summer_courses/Summer_project/mod_lammps_code/outfiles/'
    save_plots_to = '/home/elenaespinosa/OneDrive/Uni/Summer_courses/Summer_project/mod_lammps_code/plots/steady_state/'
else:
    path_to_trimmed_outfiles = '/home/s2205640/Documents/summer_project/mod_lammps_code/outfiles/' # removed the path to the TRIMMED OUTFILES!!! ### CHANGE THIS LATER ###
...
trimmed_outfiles_list_567 = [f'outfile_{i}_run_1.dat' for i in range(5, 8)] # add "trimmed_" bit at the start when doing it on trimmed outfiles!
...
def plots(data_frame_name, column_name, column_no):
    plt.figure(figsize=(16, 10))

    plt.plot(data_frames_567_trimmed[1]['Timesteps'], data_frames_567_trimmed[1][data_frame_name], marker='.', alpha=0.7, label='Model 5')
    plt.plot(data_frames_567_trimmed[2]['Timesteps'], data_frames_567_trimmed[2][data_frame_name], marker='.', alpha=0.7, label='Model 6')
    plt.plot(data_frames_567_trimmed[3]['Timesteps'], data_frames_567_trimmed[3][data_frame_name], marker='.', alpha=0.7, label='Model 7')
    ...
    plt.title(f'{column_name} vs Timesteps', fontsize ='16')
    plt.ticklabel_format(style='plain')

    plt.legend(fontsize="14")
    plt.grid(True)

    plt.savefig(save_plots_to + f"plot_{column_no}_model_5_run_1.png", dpi='figure')
    #plt.show()

for i, column in enumerate(column_name):
    plots(data_frame_name[i], column_name[i], (i+1))





def plots_models_567(mean_567, std_567, sem_567, column_name, column_no):
    ...
    plt.savefig(save_plots_to + f"plot_{column_no}_model_567_SS_run_1.png", dpi='figure')
    #plt.show()



# plots 1 to 8 for models 5, 6 and 7 - Mean ± 1 SEM (left) and STD (right)
"""
for i, column in enumerate(column_name):
    mean_567, std_567, sem_567 = get_stats_567(data_frames_567_trimmed, data_frame_name[i])
    plots_models_567(mean_567, std_567, sem_567, column_name[i], (i+1))
"""
