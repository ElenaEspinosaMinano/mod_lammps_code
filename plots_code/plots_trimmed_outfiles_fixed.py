...
# get stats for a particular dataframe and column + append it to a list
...
# plot 1 - Number of clusters vs Timesteps - Mean + SEM
...
for i in range(1, 4):
    ...
    plt.bar(data_frames_123_trimmed[i]['Timesteps'], data_frames_123_trimmed[i]['No_of_clusters'], yerr=2*sem, fmt='', ecolor=colors[i-1],
...
mean_1, std_1, sem_1 = get_stats(data_frames_123_trimmed, 'No_of_clusters')
...
left_bar_1 = axs[0].bar(models, mean_1, yerr=sem_1, color=colors[:4])
axs[0].bar_label(left_bar_1, labels=[f'{m:.2f}±{sem_1[i]:.2f}' for i, m in enumerate(mean_1)], fontsize=10)
...
right_bar_1 = axs[1].bar(models, std_1, color=colors[:4])
axs[1].bar_label(right_bar_1, labels=[f'{s:.2f}± ---' for i, s in enumerate(std_1)], fontsize=10)
