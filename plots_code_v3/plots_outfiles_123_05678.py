...
if local_or_cplab == "local":
    ...
    save_plots_to = '/home/elenaespinosa/OneDrive/Uni/Summer_courses/Summer_project/mod_lammps_code/plots_v3/'
    save_plots_to_SS = '/home/elenaespinosa/OneDrive/Uni/Summer_courses/Summer_project/mod_lammps_code/plots_v3/steady_state/'
    save_plots_to_hists = '/home/elenaespinosa/OneDrive/Uni/Summer_courses/Summer_project/mod_lammps_code/plots_v3/hists/'
    ...
    save_plots_to = '/home/s2205640/Documents/summer_project/mod_lammps_code/plots_v3/'
    save_plots_to_SS = '/home/s2205640/Documents/summer_project/mod_lammps_code/plots_v3/steady_state/'
    save_plots_to_hists = '/home/s2205640/Documents/summer_project/mod_lammps_code/plots_v3/hists/'


### -----------------------------------------------------------------------------------------------------------------------------

###
#   Functions - calc_stats, get_stats_123, get_stats_05678, lines_in_file, count_cluster_sizes, get_counts_and_sizes
###
...
###
#   First investigation - Model 1, 2 + 3 plots - proteins being type 4, clusters defined as > 1 protein + ct 2.3
###

### -----------------------------------------------------------------------------------------------------------------------------

outfiles_list_123_v3 = [f'outfile_{i}_run_1_v3.dat' for i in range(1, 4)]
...
# parse data
for i in range(1, 4):
    data_frames_123_v3[i] = pd.read_csv(path_to_outfiles + outfiles_list_123_v3[i-1], sep=' ', comment='#', header=None)
    data_frames_123_v3[i].columns = ['Timesteps', 'No_of_clusters', 'Mean_size_of_clusters', 
                                        'Size_of_largest_cluster', 'No_of_clusters_of_size_1', 'No_proteins_bound_to_poly', 
                                        'Fraction_clusters_bound_to_poly', 'No_type_2_poly_bound_to_prot', 'Mean_no_type_2_in_cluster']


def full_plots_123(data_frame_name, column_name, column_no):
    plt.figure(figsize=(16, 10))

    for i in range(1, 4):
        plt.plot(data_frames_123_v3[i]['Timesteps'], data_frames_123_v3[i][data_frame_name], marker='.', alpha=0.7, label=f'Model {models_123[i-1]}')
    ...
    plt.savefig(save_plots_to + f"plot_{column_no}_model_123_run_1_v3.png", dpi='figure')
    plt.show()

# full outfile plots for models 123
"""
for i, column in enumerate(column_name):
    full_plots_123(data_frame_name[i], column_name[i], (i+1))
"""
...
def plots_models_123(mean_123, std_123, sem_123, column_name, column_no):
    bar_labels_mean = [f'Model {model}: {mean_123[i]:.2f} ± {sem_123[i]:.2f} (1 SEM)' for i, model in enumerate(models_123)]
    bar_labels_std = [f'Model {model}: {std_123[i]:.2f} ± ---' for i, model in enumerate(models_123)]
    model_labels = [f'Model {model}' for model in (models_123)]
...
# plots 1 to 8 for models 1, 2 and 3 - Mean ± 1 SEM (left) and STD (right) version 3
"""
for i, column in enumerate(column_name):
